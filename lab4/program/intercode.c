...
InterCodes *translate_Exp(struct node *root, Operand op) {
	...
	if (strcmp(child->type, "INT") == 0) {
		if (op->kind != VARIABLE) {
			op->kind = CONSTINT;
			op->u.value.Int = atoi(child->value);
			return NULL;
		} else {
	...
	if (strcmp(child->type, "FLOAT") == 0) {
		if (op->kind != VARIABLE) {
			op->kind = CONSTFLOAT;
			op->u.value.Float = atof(child->value);
			return NULL;
		} else {
	...
	if (strcmp(child->type, "Exp") == 0) {
		...
		if (strcmp(child->type, "ASSIGNOP") == 0) {
			InterCodes *code1 = NULL;// = translate_Exp(child->next, tmp);
			...
			if (strcmp(now->child->type, "ID") == 0) {
				...
				code1 = translate_Exp(child->next, OP);
				if (op != NULL) {
				...
				Operand tmp = newOperand(TEMPVAR, NULL);
				code1 = translate_Exp(child->next, tmp);
		...
		if (strcmp(child->type, "LB") == 0) {
			...
			if (op->kind != VARIABLE) {
				code3 = newInterCode(ARRAY, op4, op2, op);
				op->kind = TEMPVAR_;
			} else {
				Operand op5 = newOperand(TEMPVAR_, NULL);
				code3 = newInterCode(ARRAY, op4, op2, op5);
				code3 = bindCode(code3, newInterCode(ASSIGN, op, op5, NULL));
			}
			return bindCode(bindCode(code1, code2), code3);
		}
...
void printCode(InterCodes *st, FILE *fp) {
	while (st != NULL) {
...
void printC(InterCodes *st, FILE *fp) {
	while (st != NULL) {
		if (st->code.kind == ASSIGN) {
			char *s1 = change(st->code.u.assign.left), 
				 *s2 = change(st->code.u.assign.right);
...
int find(char *value, int x) {
	union { int vi; float vf; } u;
	if (x == 1) u.vi = atoi(value);
	else u.vf = atof(value);

	int i;
	for (i=0; i<tmpvarcount; i++) 
		if (store[i] != NULL) {
			if (store[i]->kind == CONSTINT && store[i]->u.value.Int == u.vi) return i;
			if (store[i]->kind == CONSTFLOAT && store[i]->u.value.Float == u.vf) return i;
		}
	return -1;
}

void optimal(InterCodes *st) {
	InterCodes *tmp = st;
	while (st != NULL) {
		if (st->code.kind == CONDITION) {
			InterCodes *c1 = st, *c2 = st->next;
			if (c2 == NULL) continue;
			InterCodes *c3 = c2->next;
			if (c3 == NULL) continue;
			if (c2->code.kind == GOTOL && c3->code.kind == LABEL && 
				c1->code.u.relop.label->u.var_no == c3->code.u.op->u.var_no) {
				c1->code.u.relop.label = c2->code.u.op;
				delCode(c2);
				char ori[3], tar[3]; 
				strcpy(ori, c1->code.u.relop.rel->u.value.relop);
				if (strcmp(ori, ">") == 0) 
					strcpy(tar, "<=");
					...
					strcpy(tar, ">");
				strcpy(c1->code.u.relop.rel->u.value.relop, tar);
			}
		} else if (st->code.kind == GOTOL) {
			InterCodes *c1 = st, *c2 = st->next;
			if (c2 != NULL && c2->code.kind == LABEL && 
				c1->code.u.op == c2->code.u.op) {
				st = st->next;
				delCode(c1);
			}
		}
...
void removeLabel(InterCodes *st) {
	int use[1<<10];
	memset(use, 0, sizeof(use));

	InterCodes *tmp = st;
	while (st != NULL) {
		if (st->code.kind == GOTOL) 
			use[st->code.u.op->u.var_no] = 1;
		if (st->code.kind == CONDITION)
			use[st->code.u.relop.label->u.var_no] = 1; 
		st = st->next;
	}
	st = tmp;
	int num;
	while (st != NULL) {
		if (st->code.kind == LABEL) { 
			if (use[st->code.u.op->u.var_no] == 0) { 
				InterCodes *t = st;
				st = st->next;
				delCode(t);
			} else st = st->next;
...
int same(InterCodes *c1, InterCodes *c2) {
	if (c1->code.kind >= ADD && c1->code.kind <= DIV && 
		c2->code.kind >= ADD && c2->code.kind <= DIV && c1->code.kind == c2->code.kind) {

		if (c1->code.u.binop.result->kind != TEMPVAR ||
			c2->code.u.binop.result->kind != TEMPVAR) return 0;

		if (c1->code.u.binop.op1->u.var_no == c2->code.u.binop.op1->u.var_no &&
			c1->code.u.binop.op2->u.var_no == c2->code.u.binop.op2->u.var_no) return 1;
	}

	return 0;
}

void removeDul(InterCodes *c) {
	InterCodes *now = c;
	while (now != NULL) {
		InterCodes *bef = c;
		int flag = 0;
		while (bef != now) {
			if (same(now, bef)) { flag = 1; break; }
			bef = bef->next;
		}
		if (flag) {
			InterCodes *tmp = now;
			now = now->next;
			tmp->code.u.binop.result->u.var_no = bef->code.u.binop.result->u.var_no;
			delCode(tmp);
		} else 
			now = now->next;
	}
...
void delCode(InterCodes *c) {
	InterCodes *head = c->prev, *tail = c->next;
	if (c == st) {
		st = st->next; return ;
	}
	if (head != NULL) 
